package kabam.rotmg.packages
{
   ...
   public class PackageConfig implements IConfig
   {
      ...
      {
         this.injector.map(Packages).asSingleton();
         ...
         this.commandMap.map(BuyPackageSignal).toCommand(BuyPackageCommand).withGuards(IsAccountRegisteredGuard,IsPackageAffordableGuard);
         ...
         this.commandMap.map(OpenPackageSignal).toCommand(OpenPackageCommand).withGuards(IsAccountRegisteredGuard);
